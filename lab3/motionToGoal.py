...
def motionToGoal(distanceError, motorControl):
    ips = KP * distanceError
    if ips > 6:
        ips = 6
    if ips < -6:
...
def run(motorControl, tof, camera):
    ...
    while True:
        ...
        distanceFromFront = tof.getFrontDistance()
        rotateError = xPoint - CENTER_X_COORD
        frontDistanceError = distanceFromFront - 5
        rotateThreshold = 150
        # if distanceFromFront < 15:
        #     rotateThreshold = 400

        if abs(frontDistanceError) > 2:
            if abs(rotateError) > rotateThreshold:
                faceGoal(rotateError, motorControl)
            else:
                motionToGoal(frontDistanceError, motorControl)
        else:
